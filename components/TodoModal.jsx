...
export default function TodoModal({ list, closeModal }) {
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      ...
      if (docSnap.exists) {
        const taskData = docSnap.data()[list.id];
        if (taskData) {
          setTodos(taskData[0]?.todos || []);
        }
      }
    };
    fetchTodos();
  ...
  const toggleTodoCompleted = (index) => {
    ...
    setTodos(newTodos);
    updateTodosInFirestore(newTodos);
  };

  const addTodo = () => {
    if (newTodo.trim().length > 0) {
      const newTodos = [...todos, { title: newTodo.trim(), completed: false }];
      setTodos(newTodos);
      setNewTodo("");
      updateTodosInFirestore(newTodos);
      Keyboard.dismiss();
 ...
 const updateTodosInFirestore = async (updatedTodos) => {
   try {
     const docRef = doc(db, "notes", user.uid);
     const taskToUpdate = {
       name: list.name,
     ...
     // Update Firestore with the current todos state
     ...
     Alert.alert("Error", "Failed to update todos in Firestore.");
   }
 };
...
const styles = StyleSheet.create({
  ...
  closeButton: { position: "absolute", top: 64, right: 32, zIndex: 10 },
  ...
  completedTodo: { textDecorationLine: "line-through", color: Colors.gray },
  checkbox: { width: 24, height: 24, borderWidth: 2, borderRadius: 6 },
  todos: { marginVertical: 8 },
  leftAction: { flex: 1, backgroundColor: "#dd2c00", justifyContent: "center" },
  rightAction: {
    flex: 1,
